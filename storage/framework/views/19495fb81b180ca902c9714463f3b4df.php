...
<header class="w-full bg-white dark:bg-gray-800 shadow px-6 py-3 flex justify-between items-center">
    ...
    <div>
        <h1 class="text-2xl font-semibold">
            <?php echo e(ucfirst( last(explode('.', request()->route()->getName())) )); ?>

        </h1>
        <p class="text-sm text-gray-500 dark:text-gray-400">
            Logged in as: <?php echo e($staff?->name); ?> (<?php echo e($roleName); ?>)
        </p>
    </div>

    <div class="flex items-center space-x-4" x-data="{ openNotif:false, notif:[], searchResults:[], searching:false, query:'', init(){ setInterval(() => { fetch('<?php echo e(route('admin.notif.poll')); ?>').then(res => res.json()).then(data => { this.notif = data.map(n => ({ title: n.action ?? 'Activity', time: new Date(n.created_at).toLocaleTimeString() })); }); }, 5000); } }" x-init="init()">
        ...
        <div class="flex items-center space-x-2">
            <div
                ...
                <?php echo e(strtoupper(substr($staff?->name ?? 'A', 0, 1))); ?>

            </div>

            <form action="<?php echo e(route('staff.logout')); ?>" method="POST">
                <?php echo csrf_field(); ?>
...
<?php /**PATH C:\xampp\htdocs\xeon\resources\views/components/admin/topbar.blade.php ENDPATH**/ ?>