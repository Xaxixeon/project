...
<header class="border-b border-slate-800 bg-slate-950/80 backdrop-blur">
    <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('home')); ?>" class="hidden md:inline-block text-base text-slate-300 hover:text-sky-400">
                Beranda
            </a>
        ...
        <section class="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-xl">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                    <a href="<?php echo e(route('products.index', array_merge(request()->all(), ['view' => 'grid']))); ?>"
                       class="px-3 py-1.5 rounded-full border <?php echo e($viewMode === 'grid' ? 'border-sky-500 text-sky-300' : 'border-slate-700 text-slate-300'); ?> text-xs">
                        Grid
                    </a>
                    <a href="<?php echo e(route('products.index', array_merge(request()->all(), ['view' => 'list']))); ?>"
                       class="px-3 py-1.5 rounded-full border <?php echo e($viewMode === 'list' ? 'border-sky-500 text-sky-300' : 'border-slate-700 text-slate-300'); ?> text-xs">
                        List
                    </a>
                </div>
            </div>
            ...
            <form method="GET" action="<?php echo e(route('products.index')); ?>"
                ...
                <div>
                    ...
                    <select name="display_group"
                        ...
                        <?php $__currentLoopData = $displayGroups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($group->id); ?>" <?php if(request('display_group') == $group->id): echo 'selected'; endif; ?>>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        ...
                        <input type="number" name="price_min" value="<?php echo e(request('price_min')); ?>"
                               class="w-full rounded-2xl border border-slate-700 bg-slate-950/70 px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                    </div>
                    <div>
                        <label class="block text-sm text-slate-400 mb-1">Harga max</label>
                        <input type="number" name="price_max" value="<?php echo e(request('price_max')); ?>"
                               class="w-full rounded-2xl border border-slate-700 bg-slate-950/70 px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                    </div>
                </div>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row gap-4 hover:border-sky-500/60 hover:shadow-xl hover:shadow-sky-500/10 transition">
                            <div class="flex-1 space-y-2">
                                ...
                                <?php if($product->variants->count()): ?>
                                    <div class="flex flex-wrap gap-1 text-[11px] text-slate-400">
                                        <?php $__currentLoopData = $product->variants->take(6); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $variant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <span class="px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700">
                                                <?php echo e($variant->size_code ?: $variant->label); ?>

                                            </span>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        <?php if($product->variants->count() > 6): ?>
                                            <span class="text-[11px] text-slate-500">+<?php echo e($product->variants->count() - 6); ?> varian</span>
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <div class="flex md:flex-col justify-between items-end md:items-end gap-3">
                                ...
                                <a href="<?php echo e(route('product.show', $product->sku)); ?>"
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between hover:border-sky-500/60 hover:-translate-y-1 hover:shadow-xl hover:shadow-sky-500/10 transition">
                            <div>
                                ...
                                <h3 class="text-sm font-semibold mb-1"><?php echo e($product->name); ?></h3>
                                ...
                                <?php if($product->variants->count()): ?>
                                    <div class="flex flex-wrap gap-1 mb-2 text-[11px] text-slate-400">
                                        <?php $__currentLoopData = $product->variants->take(3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $variant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <span class="px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700">
                                                <?php echo e($variant->size_code ?: $variant->label); ?>

                                            </span>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        <?php if($product->variants->count() > 3): ?>
                                            <span class="text-[11px] text-slate-500">+<?php echo e($product->variants->count() - 3); ?> varian</span>
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
                            </div>
...
<?php /**PATH C:\xampp\htdocs\xeon\resources\views/products/index.blade.php ENDPATH**/ ?>